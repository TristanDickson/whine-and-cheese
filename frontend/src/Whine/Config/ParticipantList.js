...
class SetPane extends Component {
  ...
  removeFromSet(set_id, set_participant_id) {
    this.props.removeParticipantFromSet(set_id, set_participant_id);
  }
  ...
  render() {
    ...
    return sets_participants
      .filter(set_participant => set_participant.set_id === set_id)
      .map(set_participant => (
        <Fragment>
          <Grid item xs={10}>
            <Typography className={classes.typography} key={set_participant._id}>
              {[
                participants.find(participant => participant._id === set_participant.participant_id)
